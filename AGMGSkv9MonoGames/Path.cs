...
public class Path : DrawableGameComponent {
   ...
   public Path(Stage theStage, List<NavNode> aPath, PathType aPathType) : base(theStage) {
	...
	public Path(Stage theStage, int[,] pathNode, PathType aPathType)
        ...
        // TO DO: Want to change initial node, so that we start at a random location. -JC
		nextNode = 0;
		pathType = aPathType;
		stage = theStage;
		done = false;
   ...
   public NavNode NextNode {
      get {
         NavNode n = null;
          ...
          // TO DO: add a flag here which is randomly assigned a value between 0 and 1 and use it to determine
          // whether or not to reverse the path.  This should result in a random direction at the start
          // b/c we are going to start at a random node and have a random chance of going forward or backward
          // as per the requirements. -JC
